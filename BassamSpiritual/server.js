...
// أ. مسار جلب كافة طلبات المستفيدين بدقة هندسية مزدوجة لمنع تشوه التفاصيل بالواجهة
...
// 1. قبول الحالة المبدئي لتبدأ إجراءات الدفع والتأكيد الكشفية
...
// 2. رفض استقبال الحالة مباشرة واعتذار كلي دون إلزام مالي
    ...
    try {
        ...
        await pool.query(
            `UPDATE requests 
             ...
             WHERE id = $7`,
            [treatmentPlan, addCost, days, expiryDate, 'completed', 0, req.params.id] 
        );
...
// 🛠️ [مسار المقال الفردي الحاسم]: يجلب كامل النص للمقال المحدد بدون التأثير على اللوحة
