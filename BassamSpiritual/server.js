...
const initializeDatabase = async () => {
    try {
        ...
        if (parseInt(aiCount.rows[0].count) === 0) {
            await pool.query(`INSERT INTO ai_config (instructions) VALUES ($1)`, ["أنت مستشار فقهي وروحاني معتمد في مركز النور الرباني التابع للشيخ بسام. أجب على استفسارات الزوار بلطف وأدب جم."]);
        }
...
app.post('/api/dashboard/request', authenticateToken, async (req, res) => {
    ...
    try {
        const user = await pool.query(`SELECT full_name, email, phone FROM users WHERE id = $1`, [req.user.id]);
        await pool.query(`INSERT INTO requests (user_id, fullName, email, userPhone, serviceType, description, status) VALUES ($1, $2, $3, $4, $5, $6, 'pending')`, [req.user.id, user.rows[0].full_name, user.rows[0].email, user.rows[0].phone, serviceType, description]);
        res.json({ success: true });
...
app.get('/api/dashboard/request/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (parseInt(result.rows[0].user_id) !== parseInt(req.user.id) && req.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح.' });
...
app.put('/api/dashboard/request/:id/submit-payment', authenticateToken, async (req, res) => {
    ...
    try {
        await pool.query(`UPDATE requests SET paymentMethod=$1, payment_sender_name=$2, payment_transfer_number=$3, status='payment_submitted' WHERE id=$4 AND user_id=$5`, [paymentMethod, paymentSenderName, paymentTransferNumber, req.params.id, req.user.id]);
        res.json({ success: true });
...
app.delete('/api/admin/articles/:id', authenticateToken, requireAdmin, async (req, res) => { try { await pool.query(`DELETE FROM articles WHERE id = $1`, [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: 'خطأ في حذف المقال.' }); } });
app.get('/api/reviews', async (req, res) => { try { const result = await pool.query(`SELECT * FROM reviews WHERE "isApproved" = TRUE ORDER BY "createdAt" DESC`); res.json(result.rows); } catch (e) { res.status(500).json({ error: 'خطأ في تحميل التقييمات.' }); } });
app.get('/api/admin/reviews', authenticateToken, requireAdmin, async (req, res) => { try { const result = await pool.query(`SELECT * FROM reviews ORDER BY "createdAt" DESC`); res.json({ success: true, reviews: result.rows }); } catch (e) { res.status(500).json({ error: 'خطأ في تحميل المراجعات.' }); } });
app.put('/api/admin/reviews/:id', authenticateToken, requireAdmin, async (req, res) => { try { await pool.query(`UPDATE reviews SET "isApproved" = $1 WHERE id = $2`, [req.body.isApproved, req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: 'خطأ.' }); } });
app.delete('/api/admin/reviews/:id', authenticateToken, requireAdmin, async (req, res) => { try { await pool.query(`DELETE FROM reviews WHERE id = $1`, [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: 'خطأ.' }); } });
app.get('/api/admin/ai-instructions', authenticateToken, requireAdmin, async (req, res) => { try { const result = await pool.query(`SELECT instructions FROM ai_config ORDER BY id DESC LIMIT 1`); res.json({ success: true, instructions: result.rows[0]?.instructions || '' }); } catch (e) { res.status(500).json({ error: 'خطأ.' }); } });
app.put('/api/admin/ai-instructions', authenticateToken, requireAdmin, async (req, res) => { try { await pool.query(`UPDATE ai_config SET instructions = $1`, [req.body.instructions]); res.json({ success: true }); } catch (e) { res.status(500).json({ error: 'خطأ.' }); } });
...
app.get('/force-reset-admin', async (req, res) => {
    ...
    try {
        ...
        await pool.query(`INSERT INTO users (full_name, email, password, role) VALUES ($1, $2, $3, $4)`, ['الشيخ بسام', email, hashed, 'admin']);
        const token = jwt.sign({ id: 0, email: email, role: 'admin', full_name: 'الشيخ بسام' }, JWT_SECRET, { expiresIn: '24h' });
        res.send(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>تم بنجاح</title></head><body style="font-family: sans-serif; text-align: center; padding: 50px;"><h1>✅ تم ضبط حساب المدير بنجاح</h1><p>يتم الآن توجيهك إلى لوحة التحكم...</p><script>localStorage.setItem('bassam_auth_token', '${token}');localStorage.setItem('bassam_user', JSON.stringify({ full_name: 'الشيخ بسام', email: '${email}', role: 'admin' }));setTimeout(function() { window.location.href = '/admin.html'; }, 2000);</script></body></html>`);
