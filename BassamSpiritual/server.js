...
app.put('/api/admin/requests/:id/diagnose', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        await pool.query(
            `UPDATE requests SET "initial_diagnosis" = $1, "treatment_plan" = $2, status = 'diagnosed' WHERE id = $3`,
        ...
        if (reqData.rows.length > 0) {
            ...
            await pool.query(
                `INSERT INTO messages ("requestId", "senderId", "senderName", "senderRole", "messageText") 
                 VALUES ($1, $2, $3, $4, $5)`,
                [requestId, req.user.id, req.user.full_name, 'admin', messageText]
            );
        }

        res.json({ success: true, message: 'تم التشخيص وإرسال رسالة للمستفيد.' });
    } catch (error) {
        console.error('❌ خطأ في التشخيص:', error.message);
        res.status(500).json({ success: false, error: 'حدث خطأ أثناء التشخيص.' });
    }
});
