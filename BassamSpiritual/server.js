...
// ==============================================
// 3. تهيئة وبناء الجداول تلقائياً
// ==============================================
...
app.get('/api/auth/verify', verifyToken, async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, error: 'خطأ في التحقق.' }); }
});

// ==============================================
// 6. مسارات لوحة تحكم المستفيد
// ==============================================
app.get('/api/dashboard/me', verifyToken, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            user: { id: userData.id, fullName: userData.full_name, email: userData.email, role: userData.role },
...
app.get('/api/dashboard/request/:id', verifyToken, async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ success: false, error: 'خطأ في جلب الطلب.' }); }
});
...
app.put('/api/dashboard/request/:id/submit-payment', verifyToken, async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ success: false, error: 'خطأ في حفظ بيانات الدفع.' }); }
});

// ==============================================
// 7. مسارات لوحة تحكم المدير
// ==============================================
...
app.get('/api/requests/:id/messages', verifyToken, async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ success: false, error: 'خطأ في جلب المراسلات.' }); }
});

// ==============================================
// 9. المقالات والمراجعات وتوجيه الذكاء الاصطناعي
// ==============================================
...
app.delete('/api/admin/articles/:id', verifyToken, verifyAdmin, async (req, res) => {
    try { await pool.query('DELETE FROM articles WHERE id = $1', [req.params.id]); res.json({ success: true }); } catch (e) { res.status(500).json({ success: false }); }
});
