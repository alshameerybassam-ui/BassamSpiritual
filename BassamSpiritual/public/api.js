// =================================================================
// api.js - الدماغ الواحد المطلق للمنصة (الإصدار النهائي الكامل)
// =================================================================
...
const AdminPage = {
    ...
    // --- الطلبات ---
    async loadRequests() {
        try { this.allRequests = await apiRequest('/api/admin/requests'); this.renderFilteredTable(); this.updateStats(this.allRequests); } catch (e) { showNotification('❌ تعذر تحميل الطلبات.', 'error'); }
    },
    updateStats(list) {
    ...
    renderFilteredTable() {
        ...
        if (this.currentFilter !== 'all') { if (this.currentFilter === 'processing') filtered = filtered.filter(r => r.status === 'processing' || r.status === 'completed'); else filtered = filtered.filter(r => r.status === this.currentFilter); }
        if (searchTerm) filtered = filtered.filter(r => (r.fullName || '').toLowerCase().includes(searchTerm) || (r.serviceType || '').toLowerCase().includes(searchTerm));
        this.renderTable(filtered);
    },
    renderTable(list) {
        const tbody = document.getElementById('requestsBody'); if (!tbody) return;
        if (!list.length) { tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;">لا توجد طلبات.</td></tr>'; return; }
        const map = { 'pending': 'قيد الانتظار', 'accepted_waiting_payment': 'بانتظار الدفع', 'payment_submitted': 'مراجعة الدفع', 'processing': 'قيد العلاج', 'completed': 'مكتمل', 'rejected_by_admin': 'مرفوض', 'closed': 'مغلق' };
        tbody.innerHTML = list.map((r, i) => `<tr><td>${i + 1}</td><td><strong style="color:#2C3E50; cursor:pointer; text-decoration:underline;" onclick="AdminPage.viewDetails('${r.id}')">👤 ${r.fullName || '—'}</strong></td><td>${r.email || '—'}</td><td>${r.serviceType || '—'}</td><td>${map[r.status] || r.status}</td><td>${(r.totalPaidAmount || 0) > 0 ? '💰 مدفوع' : '—'}</td><td>${r.createdAt ? new Date(r.createdAt).toLocaleDateString('ar-YE') : '—'}</td><td><button class="action-btn edit" onclick="AdminPage.viewDetails('${r.id}')">فتح</button> <button class="action-btn delete" onclick="AdminPage.deleteRequest('${r.id}')">حذف</button></td></tr>`).join('');
    },

    async viewDetails(id) {
        const modal = document.getElementById('detailsModal'); const body = document.getElementById('modalBody');
        if (!modal || !body) return;
        modal.classList.add('show'); modal.style.display = 'flex'; body.innerHTML = '<p>جاري تحميل التفاصيل...</p>';
        const req = this.allRequests.find(r => String(r.id) === String(id)) || {};
        let html = `<div style="text-align: right; direction: rtl; line-height: 1.8;"><p><strong>👤 المستفيد:</strong> ${req.fullName || '—'}</p><p><strong>📧 البريد:</strong> ${req.email || '—'}</p><p><strong>🛠 الخدمة:</strong> ${req.serviceType || '—'}</p><p><strong>📌 الحالة:</strong> ${req.status || '—'}</p><p><strong>📝 الوصف:</strong></p><div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:12px; margin-bottom:12px;">${req.description || '—'}</div>`;
        try {
            const messagesRes = await apiRequest(`/api/requests/${id}/messages`); const messages = messagesRes.messages || [];
            if (messages.length > 0) {
                html += `<strong>💬 سجل المراسلات:</strong><div style="max-height:200px; overflow-y:auto; border:1px solid #e2e8f0; border-radius:8px; padding:8px; margin-bottom:12px;">${messages.map(msg => { const isAdmin = msg.senderRole === 'admin'; return `<div style="margin-bottom:6px; padding:6px; background:${isAdmin ? '#e8f5e9' : '#f5f5f5'}; border-radius:6px;"><small style="color:#64748b;">${isAdmin ? 'الشيخ' : 'المستفيد'} - ${new Date(msg.createdAt).toLocaleString('ar-YE')}</small><p style="margin:4px 0 0;">${msg.messageText}</p></div>`; }).join('')}</div>`;
            }
        } catch (e) {}
        html += `<div style="margin-top:15px; display:flex; gap:8px; flex-wrap:wrap;">`;
        if (req.status === 'pending') { html += `<button onclick="AdminPage.acceptRequest('${req.id}')" style="background:#2ecc71; color:#fff; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; font-family:'Cairo';">✅ قبول</button> <button onclick="AdminPage.rejectRequest('${req.id}')" style="background:#e74c3c; color:#fff; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; font-family:'Cairo';">❌ رفض</button>`; }
        else if (req.status === 'payment_submitted') { html += `<button onclick="AdminPage.approvePayment('${req.id}')" style="background:#2ecc71; color:#fff; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; font-family:'Cairo';">💰 اعتماد الدفع</button> <button onclick="AdminPage.rejectPayment('${req.id}')" style="background:#e74c3c; color:#fff; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; font-family:'Cairo';">🚫 رفض الدفع</button>`; }
        html += `</div></div>`;
        body.innerHTML = html;
    },
    ...
    async loadAdminArticles() {
        ...
        } catch (e) { container.innerHTML = '<p>تعذر تحميل المقالات.</p>'; }
    },
    openArticleForm(id = null) {
        document.getElementById('articleFormContainer').style.display = 'block';
        if (id) {
            const art = this.allArticles.find(a => a.id == id);
            if (art) { document.getElementById('adminArticleId').value = art.id; document.getElementById('adminArticleTitle').value = art.title; document.getElementById('adminArticleSummary').value = art.summary; document.getElementById('adminArticleContent').value = art.content; }
        } else {
            document.getElementById('adminArticleId').value = ''; document.getElementById('adminArticleTitle').value = ''; document.getElementById('adminArticleSummary').value = ''; document.getElementById('adminArticleContent').value = '';
        }
    },
    ...
    async saveArticle(e) {
        ...
        const summary = document.getElementById('adminArticleSummary').value;
        const content = document.getElementById('adminArticleContent').value;
        const url = id ? `/api/admin/articles/${id}` : '/api/admin/articles';
        const method = id ? 'PUT' : 'POST';
        try { await apiRequest(url, method, { title, summary, content }); showNotification('✅ تم حفظ المقال.'); this.closeArticleForm(); this.loadAdminArticles(); } catch (e) { showNotification('❌ فشل في حفظ المقال.', 'error'); }
    },
    ...
    async loadAiInstructions() {
        const container = document.getElementById('adminAiContainer'); if (!container) return;
        try {
            const data = await apiRequest('/api/admin/ai-instructions'); const instructions = data.instructions || '';
            container.innerHTML = `<h2>توجيه الذكاء الاصطناعي</h2><textarea id="aiInstructions" style="width:100%; height:200px; padding:10px; border-radius:8px; font-family:'Cairo';">${instructions}</textarea><button onclick="AdminPage.saveAiInstructions()" style="margin-top:10px; background:#2ecc71; color:#fff; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-family:'Cairo'; font-weight:600;">حفظ التوجيه</button>`;
        } catch (e) { container.innerHTML = '<p>تعذر تحميل التوجيهات.</p>'; }
    },
    async saveAiInstructions() {
        const instructions = document.getElementById('aiInstructions').value;
        try { await apiRequest('/api/admin/ai-instructions', 'PUT', { instructions }); showNotification('✅ تم حفظ توجيه الذكاء الاصطناعي.'); } catch (e) { showNotification('❌ فشل في الحفظ.', 'error'); }
    }
};
