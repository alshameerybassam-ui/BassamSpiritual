...
function startAdminDictation(btnElement, targetInputId) {
    ...
    if (!inputField) {
        showNotification('⚠️ لم يتم العثور على حقل الإدخال المطلوب.');
        return;
    }
...
async function rejectCaseFirst(id) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showNotification('🔴 تم رفض استقبال ملف الحالة واعتذر السيرفر عنها بنجاح. أغلقت الاستشارة تماماً والمستفيد غير ملزم بأي إجراءات.');
            loadRequests();
            closeDetailsModal();
        }
    } catch (e) {
...
async function submitTreatmentPlan(id) {
    ...
    if (!treatmentPlan) {
        showNotification('⚠️ يرجى صياغة الخطة العلاجية أو الأذكار أولاً.');
        return;
    }
...
async function toggleMessageLock(id, lockValue) {
    ...
    try {
        ...
        showNotification('❌ خطأ في معالجة طلب قفل المراسلات.', 'error');
    }
}

async function deleteRequest(id) {
    if (!confirm('⚠️ تنبيه حرج جداً: هل أنت متأكد من مسح ملف هذا المستفيد نهائياً؟')) return;
    const token = localStorage.getItem('token');
    try {
        ...
        showNotification('❌ تعذر الاتصال بالسيرفر لإتمام عملية المسح.', 'error');
    }
}
