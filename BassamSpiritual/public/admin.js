...
async function viewDetails(id) {
    ...
    try {
        ...
        console.error(e);
        mBody.innerHTML = '<div style="text-align:center; padding:20px; color:#e74c3c;">❌ فشل في جلب سجل محادثات وتفاصيل هذا الطلب من السيرفر.</div>';
    }
}

// ===== 9. دوال معالجة حالات الفرز والتحقق المالي المباشر بالخلفية السحابية =====
async function acceptCaseFirst(id) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showNotification('🟢 تم قبول استقبال الملف مبدئياً، وبانتظار دفع الكشفية من المستفيد.');
            document.getElementById('detailsModal').classList.remove('show');
            document.getElementById('detailsModal').style.display = 'none';
            await loadRequests();
        }
    } catch (e) {
        showNotification('❌ تعذر إرسال أمر القبول للسيرفر.', 'error');
    }
}

async function rejectCaseFirst(id) {
    const reason = prompt('يرجى كتابة سبب الاعتذار عن الملف للمستفيد:');
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showNotification('🔴 تم رفض الملف واعتذار فضيلتكم للمستفيد كلياً.');
            document.getElementById('detailsModal').classList.remove('show');
            document.getElementById('detailsModal').style.display = 'none';
            await loadRequests();
        }
    } catch (e) {
        showNotification('❌ تعذر إرسال أمر الاعتذار.', 'error');
    }
}
...
async function approvePaymentDirect(id) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showNotification('🟢 تم اعتماد قيمة الكشفية بنجاح، والملف جاهز الآن لوضع البرنامج العلاجي.');
            document.getElementById('detailsModal').classList.remove('show');
            document.getElementById('detailsModal').style.display = 'none';
            await loadRequests();
        }
    } catch (e) {
        showNotification('❌ تعذر اعتماد الحوالة.', 'error');
    }
}

async function rejectPaymentDirect(id) {
    const reason = prompt('يرجى ذكر سبب عدم مطابقة الحوالة ليعيد المستفيد رفعها:');
    if (reason === null) return;
    const token = localStorage.getItem('token');
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showNotification('🔴 تم رفض الإيصال بنجاح وتوجيه المستفيد لتعديل البيانات المرفوعة.');
            document.getElementById('detailsModal').classList.remove('show');
            document.getElementById('detailsModal').style.display = 'none';
            await loadRequests();
        }
    } catch (e) {
        showNotification('❌ تعذر إرسال الرفض المالي.', 'error');
    }
}
